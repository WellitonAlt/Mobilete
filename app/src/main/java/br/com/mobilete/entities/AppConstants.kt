package br.com.mobilete.entities

object AppConstants{
    const val REQUEST_CAMERA: Int = 10
    const val REQUEST_GALERIA: Int = 20
    const val REQUEST_LOCATION: Int = 30
    const val REQUEST_PLACE_PICKER: Int = 40
    const val REQUEST_EXTERNAL: Int = 50

    const val DELETA: Int = 1
    const val EDITA: Int = 2

    const val TAG_CAD: String = "FirebaseLog - Salva no Banco"
    const val TAG_UP: String = "FirebaseLog - Upload Foto"
    const val TAG_AUTH: String = "FirebaseLog - Cria Autenticação"
    const val TAG_USUARIO: String = "FirebaseLog - Recupera Usuario"
    const val TAG_PHOTO: String = "FirebaseLog - Recupera Foto"
    const val TAG_ANUNCIO: String = "FirebaseLog - Recupera Anuncios"
    const val TAG_ANUNCIO_DELETE: String = "FirebaseLog - Deleta Anuncios"
    const val TAG_ANUNCIO_FOTO: String = "FirebaseLog - Deleta Anuncios Foto"

    const val TAG_LOGIN_FB: String = "LoginLog - Facebook"
    const val TAG_LOGIN_EMAIL_SENHA: String = "LoginLog - Email Senha"

    const val TAG_LOC = "LocationLog - "
    const val TAG_MAP = "GoogleMap - "

    const val FIREBASE: String = "Firebase"
    const val FACEBOOK: String = "Facebook"

    const val LOGIN: String = "Login"
    const val ANUNCIO: String = "Anuncio"
    const val MEUS_ANUNCIO: String = "MeusAnuncio"
    const val SOBRE: String = "Sobre"


}